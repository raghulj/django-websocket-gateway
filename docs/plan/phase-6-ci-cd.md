...
- [ ] **Required check:** branch protection on `main` requires all three jobs.

### 6.2 `.github/workflows/release.yml`
- [ ] **Trigger:** `push` of a tag matching `v*.*.*`.
- [ ] **Jobs (sequential, fan-out where useful):**
...
- [ ] **Version sync:** the python-publish job edits `_version.py` in-tree before building. Do NOT commit that change back to `main`; it's tag-scoped.
- [ ] **Provenance:** include `--provenance=true` on the docker build (free attestation).

### 6.3 `.github/workflows/docs.yml`
- [ ] Use the sketch in `/CLAUDE.md` Step 23 verbatim. Pin actions to versions, not `@main`.
- [ ] Add a `concurrency: { group: pages, cancel-in-progress: true }` to avoid races on rapid pushes.
